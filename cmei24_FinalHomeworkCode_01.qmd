...
`Yong: I really appreciated that you included a step to remove punctuation before splitting the text. This extra cleaning step ensures that words are not attached to punctuation, which is a nice improvement over my original approach where I didn’t remove punctuation.`
...
`Yong: Your code is clearly organized into distinct layers (“Layer 2,” “Layer 1,” and the overall “Clade”), which makes the hierarchical relationships easy to follow.`
...
`Yong: While you use $X2 and $X3 for column coercion, my code used numeric indexing (e.g., df_m3[, 2]). Both work well, but you might consider adding a brief comment on why you chose to use the named columns. This might make it easier for reader to understand.`
...
```{r}
df3 <- data.frame(m3)
# Once the matrix is parsed into a data frame, it creates one that has no column titles, but I can still call them by X1, X2, and X3
```

As a data frame, coerce the second column to be *logical-valued*

```{r}
# While it is possible to use numeric indexing like df_m3[, 2] as you would in a matrix, I decided to use column names since this is good practice for future data frame column manipulation since we will most likely be using column names anyway!
...
`Yong: The following original code chunk will give an error as the file is being loaded with absolute path which will only work with your device. What I would suggested is to use relative path as long as the qmd/rmd is in the same folder with the data.`

```{r, eval=FALSE}
require(readr)
# The original code had my computer path but I fixed it to look for the file within the Github repo
cd_file <- "Country-Data-2016.txt"
...
`Yong: The following will work on my device:)`
...
k_and_c_path <- "KamilarAndCooperData.xlsx"
